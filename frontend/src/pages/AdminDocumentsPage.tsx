...
export default function AdminDocumentsPage() {
  ...
  const [documentType, setDocumentType] = useState("test_policy");
  const [policyName, setPolicyName] = useState("Safe Test Policy 2026");
  const [departmentOwner, setDepartmentOwner] = useState("Human Resources");
  const [accessLevel, setAccessLevel] = useState("all_employees");
  ...
  async function handleUpload(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`Uploaded successfully: ${uploaded.file_name}`);
      setFile(null);
      await loadDocuments();
    } catch (err) {
      setError(getApiErrorMessage(err));
  ...
  async function handleIndex(documentId: number) {
    ...
    try {
      const response = await indexDocument(documentId, true);
      setMessage(response.message || `Document ${documentId} indexed.`);
      await loadDocuments();
    } catch (err) {
      setError(getApiErrorMessage(err));
      await loadDocuments();
        ...
        <section className="main-panel">
          <header className="topbar">
            <div>
              ...
              <p>You do not have permission to access document administration.</p>
            </div>
          ...
          <section className="placeholder-card">
            ...
            <p>
              Please login as HR Admin, Finance Admin, IT Admin, or Super Admin
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <Link className="active" to="/admin/documents">
            <FileText size={18} />
            Documents
          </Link>
        </nav>
      ...
      <section className="main-panel admin-panel">
        <header className="topbar">
          <div>
            ...
            <p>
              Upload, validate, index, and manage employee policy documents.
            </p>
          </div>
          ...
          <section className="documents-card">
            <div className="documents-header">
              <div>
                ...
                <p>{documents.length} document records</p>
              </div>
                  ...
                  <tbody>
                    {documents.map((document) => (
                      <tr key={document.id}>
                        <td>{document.id}</td>
                        <td>
                          <strong>{document.policy_name}</strong>
